...
function Products() {
  ...
  const getCategory = () => {
    axios
      .get("http://localhost:1337/api/categories/" + Number(params.id))
  ...
  useEffect(() => {
    axios
      .get(PRODUCTS + "&pagination[page]=1&pagination[pageSize]=100")
  ...
  useEffect(() => {
    ...
    setCatProduct(
      products.filter(
        (product) => product.attributes.category.data.id === Number(params.id)
      )
    );
  }, [params, products]);
