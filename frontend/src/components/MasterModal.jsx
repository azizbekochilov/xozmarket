...
function MasterMadal({ master, setShow }) {
  function MasterMadal({ master, setShow, setMasters }) {
    ...
    const orderMaster = () => {
      axios
        .post(
          "http://localhost:1337/api/order-masters?populate=user&populate=master",
          {
            data: {
              user: customer,
              master: master,
        ...
        .then((res) => {
          axios
            .put(`http://localhost:1337/api/masters/${master.id}`, {
              data: {
                isBusy: true,
              },
            })
    ...
    return (
      <div class="shadow">
        <div class="modal">
          <img
            src={`http://localhost:1337${master.attributes.image.data.attributes.url}`}
            alt=""
          />
          <div class="modal-content">
            <p class="close" onClick={() => setShow(false)}>
              x
            </p>
            ...
            <br />
            <p className="p11">
              Тип работы: <span className="span">{master.attributes.kind}</span>{" "}
            </p>
            ...
            <div className="modal-content_btn">
              <button className="content_btn1" onClick={() => decrease()}>
                -
              </button>
              <h1 className="content_h1">{days}</h1>
              <button className="content_btn1" onClick={() => increase()}>
                +
              </button>
            </div>
            ...
            <div class="modal-content__bottom">
              ...
              <button
                class="modal-content__bottom-button"
                disabled={master.attributes.isBusy}
                onClick={() => orderMaster()}
              >
                Заказать
              </button>
